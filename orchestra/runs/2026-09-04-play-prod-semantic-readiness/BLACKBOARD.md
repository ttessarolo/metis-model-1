# Play-prod semantic readiness for Metis Brain

## Objective

Remove the semantic-authority blocker exposed by the sealed 10+10 headless
qualification, including structural Metis/catalog changes when evidence proves
they are required, then replay the exact qualification without weakening any
grounding, compiler, tenant or no-Apply gate.

## Baseline and authority

- Model 1: `/Users/tommasotessarolo/Developer/metis-model-1`, `main`, baseline
  `834348b8ce438cc0484105ba03296416855a0aab`, clean/aligned at preflight.
- Play-prod tenant: `/Users/tommasotessarolo/Developer/metis-tenant-play-prod`,
  `main`, baseline `5f56bdfe27e3fb00b735db630a4eb5cdf5ab12c3`, tree
  `03abf1a30603ff6cb59d55c32c3395cef868a218`, clean/aligned at preflight.
- Metis: `/Users/tommasotessarolo/Developer/ares-matioska/metis`, `main`,
  baseline `09fbf2b1ea3950519ff4051ae19d4f67eee1eddf`, tree
  `0ed6543ac6624fb3608de0830c7f328f066753e4`, clean/aligned at preflight.
- Reviewed semantic descriptions and declared tenant sources are semantic
  authority; positional catalog choice, generated reviewed prose and silent
  cross-tenant copying are forbidden.

## Writable surfaces

- This board/ledger and bounded Model 1 retrieval/preflight/tests/docs.
- Metis grammar/tooling/tests only if the first-class inheritance audit proves a
  structural gap; its local `AGENTS.md` and generated-state gates remain binding.
- Play-prod catalog declarations/semantic source files only after exact schema,
  provenance and same-tenant authority are established.

## Exclusions

- No VPN, credentials, keychain, `.env`, live ARES/OpenSearch, S3 or network
  corpus acquisition.
- No endpoint Apply, production mutation, model-weight/adapter change, training,
  prompt-corpus rewrite or gate relaxation.
- Other repositories and unrelated running services remain untouched.

## Exit gates

1. Exact census of owner catalogs, semantic states, domain states and compatible
   reviewed sources, with no inferred equivalence.
2. `semantics from` proven end-to-end or repaired with parser/validator/CLI/
   formatter/retrieval tests.
3. Tenant patch compiles and preserves technical catalog/endpoint behavior;
   semantic projection contains no unresolved ambiguity needed by the corpus.
4. Model 1 focused and authoritative gates pass on clean pinned state.
5. Exact sealed 10+10/40 headless replay produces a complete receipt; accuracy
   and readiness remain separate verdicts.

## Evidence wire

- FACT — The preceding complete receipt was safe but non-green: edits
  `PASS_DRAFT=0/10`, journeys converged `0/10`; `9/10` edits and all first
  create turns failed on `CLARIFICATION_TOO_MANY_OPTIONS`, while one edit
  returned `unsupported_metadata`.
- DECISION — The user explicitly authorized structural catalog enrichment in
  this wave and requested speed. Minimality means the smallest authoritative
  change that closes the measured blocker, not a shortcut around semantic truth.
- OPEN — Determine whether the already present first-class `semantics from`
  contract fully reaches schema-2 retrieval and whether play-prod has a
  compatible reviewed source it may name without cross-tenant invention.
- FACT — Schema-2 projection already consumes direct, same-tenant `semantics
  from` inheritance for catalog, field, finite-domain and value semantics.  The
  Brain runner was nevertheless loading both semantic projection and compiler
  documents with validation disabled, so an incompatible inherited field type
  could cross that boundary without diagnostics.
- FIX — `runtime/metis_brain/runner.mts` now loads schema-2 projection, compile
  and compile-structure documents with `validate: true`; projection fails
  closed on any tenant diagnostic and compiler receipts return bounded invalid
  diagnostics.  The runner binding is now
  `sha256:01c0792de5de9c3b6895361216a9b74a048dc377f075180aba9bb095e955fd9a`.
- FIX — Existing-target orchestration now scans the exact endpoint before
  retrieval with the bounded trivia/string-safe scanner.  One `from @catalog`
  becomes a volatile server-owned hint; a catalog explicitly named in the new
  instruction wins.  A multi-catalog source becomes an authoritative roster,
  not an operator ambiguity: retrieval exposes a catalog-qualified combined
  context without selecting a primary, while create/new remains fail-closed.
- FIX — Catalog ambiguity above five candidates no longer truncates or raises
  `CLARIFICATION_TOO_MANY_OPTIONS` for Turn schema 2.  Brain returns the whole
  bounded roster (`6..64`) in `catalog_refs` and accepts only one byte-exact,
  case-sensitive `{ "text": "<full-catalog-ref>" }` allow-list member.  The
  legacy 2..5 `option_ref` wire is unchanged; Turn schema 1 fails closed on an
  unrepresentable overflow.
- DONE — Brain focused roster `in=862 out=862 distinct=862 gaps=0`; all
  `tests/test_brain_*.py` passed without model, network or live tenant access.
  The cross-layer synthetic runner test proves incompatible `semantics from`
  is rejected by both semantic projection and compiler.  The upstream Metis
  `node --import tsx test/catalog-semantics-from.ts` gate also passed.
- RISK — The immutable hard-qualification v1 plan still seals the former
  runner digest.  Replaying 10+10 must first create a successor sealed plan and
  config binding for the new digest; mutating the old receipt would invalidate
  its evidence lineage.
- OPEN — `make check` passed foundation, pilot, lint and format, then its final
  isolated authority harness stopped with `test authority validation failed`
  because the shared Metis checkout had concurrent uncommitted tenant/vendor
  changes.  Rerun the complete gate after that checkout is clean/aligned.
- FIX — The source scanner now distinguishes traversal strings from predicate
  literals: valid empty strings such as defaults/metadata no longer suppress
  `from @catalog` extraction, while an empty finite predicate remains rejected.
  This closes the synthetic reproduction of the `play.similar_cinema` preflight
  finding without reading or mutating the tenant fixture in this lane.
- STOP — The current text candidate oracle still assumes exactly one endpoint
  take and one catalog source; `_prepare_output_contract` also rejects an
  existing source with multiple takes or fallback before generation.  The
  sealed complex corpus proves those are real, not hypothetical shapes.  Do
  not relax the scanner: the successor gate needs one compiler-derived,
  occurrence-aware IR manifest (`IrFetch.stageId`, source, cardinality and
  predicates) and must prove that every unrequested fetch/output occurrence is
  preserved.  L1003 deliberately stops before that cross-layer refactor.
- FACT [MECH] — Read-only review target `/Users/tommasotessarolo/Developer/metis-tenant-play-prod` advanced concurrently to `98e78407f7286d2a9ac404dceb655fd1f6a9118e` (`feat(catalogs): complete Brain semantic authority`), with exactly four files changed from `dafe36afd8cb31d1523e092a1567f5f77f096866`: `catalogs/link.metis` 56/17, `catalogs/trending.metis` 3/1, `catalogs/user_clusters.metis` 21/6, `catalogs/video.metis` 12/2. `git diff HEAD^ HEAD --check` passed and the tenant worktree is clean.
- DONE [MECH] — Local bounded compile: `npm --prefix /Users/tommasotessarolo/Developer/ares-matioska/metis/tooling run build:tenant -- --tenant /Users/tommasotessarolo/Developer/metis-tenant-play-prod --out /Users/tommasotessarolo/Developer/tenant-check.jfJIf0` passed: 170 endpoint IRs, runtime context 8 catalogs/137 fields, artifact-set SHA-256 `4728481219139c24c4566359ff7932db86ca57ab91ed1704bb8c2000f3faabfd`, materialized-fallbacks SHA-256 `08dd17e665f65db4862e732a9a456b58663aaf31a675b5f45b9868d7d2f2cbc8`.
- FACT [MECH] — `video_format` and `video_formats` each declare exactly the four literals in `lib/lists.metis:23` (`"4K HDR"`, `"4K SDR"`, `"HD"`, `"SD"`); the exclusion preset independently references both fields and the same 4K literals (`lib/presets.metis:38-42`). The enum literal set is authority-backed by the tenant list. Newly added per-value `means` prose is not textually present in `lib/lists.metis` and should be treated as generic annotation, not list-derived provenance.
- RISK [MECH] — Offline probes from `/Users/tommasotessarolo/Developer/ares-matioska/metis/tooling`: `catalog-domain.ts`, `catalog-semantic.ts`, `r8-description-invariant.ts`, `r8-semantic-surface.ts`, `catalog-sync-values-rewrite.ts`, and `catalog-sync-values-merge.ts` passed; `formatter-prettifier.ts` reported 2 failures in its stale fixture expectation (expects `video.values.metis` to be touched, while current corpus only touches `video.metis`), unrelated to tenant compile and requiring coordinator disposition before calling formatter gate green.
- FACT [L1004] — The compiler IR assigns source-order occurrence identities
  (`context.*`, `block.*.take.*`, `variant.*.take.*`, `inline.take.*`) after
  compilation.  It is therefore the first trustworthy owner of multi-block,
  multi-take and multi-catalog predicate lineage; the former text oracle cannot
  safely recover that graph.
- FIX [L1004] — Added private `compile-candidate`: one pinned, validated compiler
  execution returns the established redacted public compile receipt separately
  from a closed occurrence manifest.  The manifest contains ordered containers
  and fetches, catalog-qualified include/exclude/promote leaves with origin/ref,
  operator and value, cardinality plus guard/order/output/fallback hashes.  Raw
  IR and provenance never cross this boundary.  Bounds are 512 containers, 512
  fetches, 8192 leaves and 4 MiB; overflow or unresolved field-to-catalog lineage
  returns `invalid` with no partial endpoint, hash or manifest authority.
- FACT [L1004] — Final runner binding is
  `sha256:e63c3102187f64db4b12fb46b961ab196447c3fec0138958ffba8852da8dc670`.
  A read-only local compile of real `play.multiple_block_compleanno` returned
  `status=ok`, 14 ordered containers, 32 fetch occurrences, 334 predicate leaves
  across `play-prod-v2.users` and `play-prod-v2.video`, with neither raw IR nor
  provenance in the response.
- DONE [L1004] — Focused runner/bridge/grounding roster: `160 passed`; Ruff
  format/check, Prettier check, digest equality and `git diff --check` passed.
  Adversarial gates reject duplicated, removed or moved predicates, wrong
  catalog lineage, fetch/container reorder, preset-origin and non-literal
  create predicates without explicit reviewed authority.  No MLX, network,
  live tenant service, Apply, commit or push was executed.
- OPEN [L1004] — Integration outside this lane must call
  `BrainCompiler.compile_candidate` once, publish only `.receipt`, retain
  `.manifest` in-process, and compare it with the compiler-derived source
  manifest under the exact output/delta contract.  Legacy `compile`,
  `compile-structure` and the simple scanner remain available during that
  migration; hard qualification still uses `compile-structure` for its richer
  private structural facts.
- FACT [L1006] — The canonical tenant and the vendored Metis copy are byte-identical
  for `_tenant.metis`, `lib/lists.metis`, `lib/presets.metis` and
  `catalogs/video.metis`.  Offline semantic probes report domain `none` for
  `genere_mcm_primario`, `content_channels_rights`, `paesiorigine`, `mood`,
  `protagonistaSesso` and `protagonistaSpecie`; their endpoint literals are
  subsets/examples or context intersections, not exhaustive value authority.
- DECISION [L1006] — Do not manufacture partial enums from endpoint filters.
  Structural catalog enrichment remains authorized only when a same-tenant,
  canonical and exhaustive provider or reviewed crosswalk exists.  The four-value
  reviewed domains of `video_format` and `video_formats` are already complete and
  require no further change.  Future enrichment of the six open gaps requires an
  editorial canonical provider, not mechanical harvesting.
- DONE [L1006] — Catalog authority roster `in=8 out=8 distinct=8 gaps=0`:
  two format fields are already SAFE/complete; six questioned fields are NOT SAFE
  to materialize from current repository evidence.  No file, model, network,
  live tenant, secret, commit or push was touched by the delegated audit.
- DONE [L0] — The complete Metis `npm test` gate passed after the two stale
  enrichment-facing fixtures were repaired, including the real vendored corpus,
  formatter/value surfaces and lossless compile proof.  Commits `2810e7fe` through
  `dbbe944c` were pushed to `origin/main`; local and remote `main` now match and the
  Metis worktree is clean.  This closes the shared-toolchain dirt that previously
  invalidated Model 1's final authority harness.
- DONE [L1007] — The immutable v1 hard-qualification inputs were left byte-for-byte
  unchanged.  The read-only registry census found every v1 pin/path/identity and
  enumerated all ten create-journey fairness gaps; no model, network or tenant
  mutation was used.
- FIX [L0] — Added a separately sealed `play-prod-v2` corpus/plan pair.  It keeps
  the exact `10 edit + 10 journey / 40 create-turn` denominator, binds tenant
  `98e78407` / tree `914785f5`, and binds runner
  `sha256:e63c3102187f64db4b12fb46b961ab196447c3fec0138958ffba8852da8dc670`.
  Hidden structural expectations were moved into the inspectable Italian user
  prompts (catalog rosters, branch/take/pool counts and time windows); the TVOD
  11-genre request and oracle now agree at 11.
- FIX [L0] — `brain_hard_qualification.py` now admits only two exact immutable
  path pairs through a closed profile registry.  Mixed v1/v2 corpus-plan pairs,
  hash drift, tenant identity drift and runner drift fail closed.  Focused v2
  profile/fairness tests pass; the one temporarily excluded HTTP fixture is owned
  by the concurrent L1005 compiler-manifest migration and will be rerun after its
  handoff.
- FACT [L1005] — L0 explicitly expanded the bounded test-only writable roster to
  `tests/test_brain_session_isolation.py` after the extended Brain gate exposed
  its legacy fake compiler.  The fake now implements the same split
  receipt/private-manifest interface; the production runtime retains strict
  fail-closed behavior and has no fallback to legacy `compile` for named
  candidates.
- FIX [L1005] — Every named candidate is compiled exactly once through
  `compile_candidate`.  A first existing-source edit privately compiles its
  immutable baseline once; a proposal refinement reuses the revision-bound
  candidate manifest rather than recompiling the basis.  Create candidates use
  compiler-owned catalog/predicate adjudication.  A lossless compiler receipt is
  the only existing-edit delta authority currently admitted; without that exact
  authority, a structural delta fails closed.
- FIX [L1005] — Complex existing/basis output no longer passes through the
  singular text-derived `take` contract.  Multiple fetch occurrences, nested
  containers or any compiled fallback select `{mode: preserve}`; cardinality,
  order and fallback stay occurrence-exact.  An explicit output change is
  rejected until an occurrence-specific reviewed delta permit exists.
- FACT [L1005] — Manifests and manifest hashes live only in private `TurnRecord`
  fields (`repr=False`), are deep-copied across proposal basis/clarification
  resume, and are absent from model context, repair prompts, events, terminal
  payloads and HTTP.  Session drop and service shutdown erase both basis and
  candidate authority; hash drift makes a named proposal stale.
- DONE [L1005] — Integration roster `in=498 out=498 distinct=498 gaps=0` across
  orchestrator, turn lifecycle, compiler bridge/grounding, retrieval, server,
  answer route, Flash, renderer and session isolation.  Ruff check/format and
  `git diff --check` passed.  No MLX/model, network, live tenant, Apply, commit or
  push was executed.  The separate hard-qualification HTTP fake still exposes
  only legacy `compile`; L0 retained ownership of that already-recorded fixture
  migration and its rerun.
- FIX [L0] — Candidate manifests now hash the complete direct executable
  semantics of every Endpoint, Block and ContextTransform container while
  excluding only IR-node provenance and child collections already protected by
  ordered rosters.  The compiler-owned catalog comparison is exact rather than
  prefix-based.  Runner identity is
  `sha256:468bac7ea26b0661a24a4772332a3558f6cb3313be2600d317d5b75ae4589f37`;
  the resealed v2 plan is
  `sha256:6c5cb31e07e4d115c109789a26ae77e718165902a340eac547298cb6879d1817`.
- DONE [L0] — Focused manifest/orchestrator/qualification roster `in=378 out=378
  distinct=378 gaps=0`; Ruff, `git diff --check`, runner/plan/corpus digest and
  exact v2 profile load passed.  The independent L1008 rerun passed 421/421
  focused tests and confirmed all public/private manifest boundaries GREEN.
- FACT [L0] — A local, offline Flash-only census over the frozen 40 create
  messages completed in 88.297 s including a 6.409 s warmup: 14 structured
  intents were returned and 26 were rejected.  Several accepted intents
  classified structural phrases such as branches, paths and sequences as
  editorial concepts.  No 27B worker, network, live service, Apply or tenant
  write was used.
- STOP [L1008] — Do not spend the hour-long Model 1 qualification yet.  Three
  independent P0s make a green result either impossible or unsound: all ten
  complex existing edits are outside the one-take lossless subset and exact
  manifest preservation rejects their intentional delta; complex CREATE
  refinements lack an instruction/basis/revision-bound typed structural permit;
  and v2 turn-local `gte`/`contains_all` checks can false-green after dropping
  hidden reference equivalence.  Full-source generation is additionally bounded
  to 512 tokens, so very large endpoints need incremental EditPlan/rendering,
  not repeated whole-file regeneration.
- DECISION [L0] — This STOP is not a catalog-semantic deficit.  The completed
  L1006 authority census remains controlling: no partial enum may be fabricated
  from endpoint literals.  The successor fix is a universal, compiler-owned
  incremental delta protocol plus exact inspectable qualification oracles; only
  a subsequently observed unresolved metadata concept may reopen catalog
  enrichment.
- FIX [L0] — The uncommitted `play-prod-v2` profile is now explicitly
  diagnostic and non-promotable.  Profile eligibility is code-pinned rather
  than input-controlled; even a complete perfect v2 measurement records
  `promotion_gate.status=NOT_PROMOTABLE` and
  `qualification_green=false` because final normalized-IR equivalence is not
  required by that plan.  Focused profile, receipt and Ruff gates pass.
- FIX [L0] — CREATE proposal-basis refinements no longer receive an implicit
  structural-authority bypass.  Until a session/turn/source/edit-surface-bound
  DeltaPermit is admitted, any compiled create refinement with a basis fails
  closed as `CANDIDATE_STRUCTURE_MISMATCH`; no model retry can silently turn a
  broad grounding match into edit authority.
- FIX [L1011-L1014] — Existing-endpoint structural edits now use a private,
  compiler-owned edit surface plus a closed one-shot `DeltaPermit` bound to the
  session, turn, context/semantic revisions, source hash, endpoint, manifest and
  exact reference roles.  The lossless renderer admits only the reviewed
  primitives required by the frozen edit roster (take cardinality, final output
  limit, display label/title and block-argument list), rejects mixed or partial
  intent, and erases private authority on close/cancel/TTL.  This closes the
  former L1008 P0 for the ten existing edits; CREATE refinement remains a
  separate STOP.
- FIX [L1012] — Compiler selector provenance is stage-local while
  `activation_sha256` continues to bind the complete ancestor-to-stage guard
  chain.  Sibling HDR and SDR takes are therefore distinguishable without
  weakening their activation authority.  Final runner identity is
  `sha256:f20c5fe80547bb973b41610507157cb03942fe4736105722b6a5b8fa063c1dde`.
- FIX [L1016] — The v2 qualification route admits an edit as `PASS_DRAFT` only
  when the terminal identity is `generation_strategy=lossless_renderer` and the
  compiler returns the exact `metis-brain-structural-lossless-proof/v1` field
  roster in validate mode, bound to base/final source hashes and the oracle's
  exact touched count.  Fourteen proof mutations fail closed; v1 behavior is
  unchanged.
- FIX [L1017] — `PinnedMetisAuthority.close()` retries only bounded macOS
  `ENOTEMPTY`, retains its private temporary authority after a terminal cleanup
  failure so a later close can retry, blocks new jobs once closed, and exposes
  `TOOLCHAIN_CLEANUP_FAILED` instead of silently leaking state.  A real capsule
  build/close removed its root successfully.
- DONE [L1015] — Real read-only play-prod-v2 planner roster `in=10 out=10
  distinct=10 gaps=0`: every rendered full source is byte-identical to its
  frozen edit oracle in 58.742 s.  Case 1 selects the two stage-local HDR/SDR
  takes and the relative final limit; case 9 resolves `Avventura` as reviewed
  `play-prod-v2.video.genere_mcm` authority in the same snapshot.  Tenant HEAD
  `98e78407f7286d2a9ac404dceb655fd1f6a9118e`, tree
  `914785f55c2be453ee75a6314f4e9e77010eed25`, remained clean and unchanged.
- DECISION [L0] — No catalog patch is justified by the ten-edit gate.  The only
  semantic value transition uses an already reviewed, compiler-bound value;
  all other observed blockers were parser, selector, lifecycle or proof-route
  defects.  The L1006 prohibition on manufactured partial enums remains in
  force.
- FIX [L1018] — The repository test harness now supplies two separate immutable
  Git-archive authorities: the historical Metis pin for legacy oracle tests and
  the Brain toolchain pin for the schema-2 runner tests.  It no longer tests the
  current Brain runner against a historical archive that predates its sources.
  Catalog-roster answers also reject leading/trailing whitespace instead of
  normalizing it, preserving byte-exact case-sensitive membership.
- DONE [L1018] — Extended Brain roster `in=1120 out=1120 distinct=1120 gaps=0`.
  Repository-wide isolated gate `in=3273 out=3273 distinct=3273 gaps=0` with
  `3271 passed, 2 skipped, 0 failed` in 1906.75 s; foundation `87/87`, pilot,
  Ruff check/format and diff checks are green.  Independent final audit reports
  P0=0/P1=0 for the structural freeze.
- OPEN [L1019] — After committing this tested tree, run all ten edits through
  Brain's real loopback HTTP surface on the sealed SHA.  Require 10/10
